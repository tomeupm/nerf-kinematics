...
from nerf_model import NeRF, positional_encoding, get_rays, sample_pdf, raw2outputs
...
def render_rays(ray_batch, network_fn, network_fine, N_samples, embed_fn, embeddirs_fn,
                near=0., far=1., N_importance=0, raw_noise_std=0.):
    """
...
def train_nerf(args, imgs, poses, hwf, i_split, render_poses=None):
    ...
    for i in tqdm(range(N_iters), desc="Entrenando NeRF"):
        ...
        rays = torch.cat([batch_rays, near * torch.ones_like(batch_rays[:1]), far * torch.ones_like(batch_rays[:1])], 0)
        rays = torch.transpose(rays, 0, 1)
        
        # Renderizar
        rgb, disp, acc, extras = render_rays(rays, lambda x: model(x), lambda x: model_fine(x),
